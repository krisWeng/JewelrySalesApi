...
router.post('/addShopCar', (req, res) => {
  var sql = 'select shopcar_info.* from shopcar_info where shop_id=? and user_id=?'
  var sql2 = 'insert into shopcar_info ( shop_id, shop_num, user_id) value (?, 1, ?)'
  var sql3 = 'update shopcar_info set shopcar_info.shop_num=? where shop_id=? and user_id=?'
  var params = req.body;
  conn.query(sql, [params.shop_id, params.user_id], function(err, result) {
    if (err) {
      console.log(err);
