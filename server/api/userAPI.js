...
router.post('/findShopCar', (req, res) => {
  ...
  var sql1 = 'update shopcar_info set shop_num=(select tb1.COUNT(id) from (select COUNT(id) from shopcar_info where user_id=? group by shop_id having Count(shop_id)>1) tb1) where user_id=? and shop_id in (select shopcar_info.shop_id from shopcar_info where user_id=? group by shop_id having Count(shop_id)>1)'
